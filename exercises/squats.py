    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            try:
                ...
                angle2 = calculate_angle(righthip, rightknee, rightankle)
                # Visualize angle
            ...
            cv2.putText(image, 'SQUATS', (275, 25),
            ...
            cv2.circle(image, tuple(np.multiply(
                rightknee, [640, 480]).astype(int)), 5, (0, 0, 255), -1)
            cv2.circle(image, tuple(np.multiply(
                rightankle, [640, 480]).astype(int)), 5, (0, 0, 255), -1)

            cv2.circle(image, tuple(np.multiply(lefthip, [
                       640, 480]).astype(int)), 5, (0, 0, 255), -1)
            cv2.circle(image, tuple(np.multiply(
                leftknee, [640, 480]).astype(int)), 5, (0, 0, 255), -1)
            cv2.circle(image, tuple(np.multiply(
                leftankle, [640, 480]).astype(int)), 5, (0, 0, 255), -1)
        ...
        return "SQUATS DETECTION RESULT", counter, elapsed_time_formatted, calories_burnt
